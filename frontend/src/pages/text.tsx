...
const TextPage = () => {
  ...
  const onSubmit = useCallback(() => {
    if(canvasRef.current){
      ...
      const parts = 4
      const partLength = imageData.data.length / 4;
      (async () => {
        for(let i = 0; i < parts; i++){
          try {
            const response = await fetch(`/api/?block_number=${i}`, { method: 'POST', body: imageData.data.slice(i * partLength, (i + 1) * partLength) })
